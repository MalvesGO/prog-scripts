...
def exibir(cliente_id):
    ...
    for imovel in imoveis_dcts:
        imovel['form_edicao_path'] = router.to_path(form_edicao, imovel['id'])
        imovel['deletar_path'] = router.to_path(deletar, imovel['id'])
...
def salvar(cliente_id, **propriedades):
    cliente_chave = Cliente.get_by_id(cliente_id)
    ...
    if errors:
        ...
        cliente_imovel = ClienteImovel(origin=cliente_chave.key, destination=imovel_chave.key)
        cliente_imovel.put()
...
def form_edicao(imovel_id):
    ...
    contexto = {'salvar_path': router.to_path(editar, imovel_id),
                'imovel': imovel_dct}
    return TemplateResponse(contexto, 'clientes/imoveis.html')
...
class ClienteImovel(Arc):
    origin = ndb.KeyProperty(required=True) # Chave que irá referenciar o usuário
